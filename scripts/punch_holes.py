...
def make_progress_bar(current, total, width=BAR_WIDTH):
    ...
    empty = width - filled
    bar = f"{GREEN}{'█' * filled}{NC}{'░' * empty}"
    return f"[{bar}] {BLUE}{pct:3d}%{NC}"


def show_progress(current, total, suffix="", width=BAR_WIDTH):
    """Print an ASCII progress bar, staying on the same line."""
    bar = make_progress_bar(current, total, width)
    line = f"\r{bar}  {suffix}"
    # Pad with spaces and clip to terminal width (fallback 90)
...
def analyze_image(filepath, image_format='qcow2'):
    ...
    if image_format == 'qcow2':
        ...
        print(f"Clusters:  {total_blocks:,} (4KB blocks)")
        print()

        # Count zero blocks with progress bar
        zero_blocks = 0
        with open(filepath, 'rb') as f:
            for i in range(total_blocks):
                ...
                if all(b == 0 for b in data):
                    zero_blocks += 1
                if i % 2000 == 0 or i == total_blocks - 1:
                    show_progress(i + 1, total_blocks, f"Scanning zero blocks  {format_bytes(i * 4096)} / {format_bytes(file_size)}")

        print()

        zero_bytes = zero_blocks * 4096
        pct = zero_blocks / total_blocks * 100
        print(f"\n{GREEN}├ Zero blocks:   {zero_blocks:,} / {total_blocks:,} ({pct:.1f}%)")
        print(f"├ Zero data:     {format_bytes(zero_bytes)}")
        print(f"└ Potential:     {format_bytes(zero_bytes)}{NC}")
...
def punch_holes(filepath, image_format='qcow2'):
    ...
    zero_clusters = 0
    bytes_punched = 0
    processed = 0  # clusters past data_start
    ...
    with open(filepath, 'r+b') as f:
        for i in range(total_clusters):
            ...
            if processed % 2000 == 0 or processed == (total_clusters - data_start // cluster_size):
                suffix = f"Zero: {zero_clusters:,}  Punched: {format_bytes(bytes_punched)}"
                show_progress(processed, total_clusters - data_start // cluster_size, suffix)
