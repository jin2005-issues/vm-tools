...
convert_image() {
    ...
    # qemu-img convert with -p emits progress like:
    #     (45.10/100%) ...
    # to stderr. We parse that into our ASCII bar.
    local tmpfile=$(mktemp)
    qemu-img convert -p -O "$format" "$input" "$output" 2>"$tmpfile" &
    local pid=$!

    {
        while kill -0 $pid 2>/dev/null; do
            # Try to extract latest percentage from tmpfile
            pct=$(grep -oP '\(\K[0-9.]+(?=%)' "$tmpfile" 2>/dev/null | tail -1)
            pct=${pct%.*}       # drop decimal part
            pct=${pct:-0}
            [ "$pct" -gt 100 ] 2>/dev/null && pct=100
            bar=$(_draw_bar "${pct:-0}")
            printf "\r[${bar}] ${BLUE}%3s%%${NC}  Converting..." "$pct"
            sleep 0.3
        done
    } &
    local reader_pid=$!
